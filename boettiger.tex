...
\textbf{arbitrary windows}
While superficially simple to use, the calculation of statistics is subject to 
...
whether the consecutive averaging windows should overlap.  
...
% discusses these challenges, as does most of the others to lesser extent 
...
% Similarly the detrending approaches can remove the signal?
% Should we also mention that some popular approaches such as autocorrelation require evenly-spaced points, and interpolating is incorrect, introducing fictitious correlations?
...
3) Specifying an appropriate null model is difficult.  
% simulation of replicates under the original model
As the summary statistic approach 
the non-parametric significance test for Kendall's tau tests against the null model that the sequence is independent,
...
or may not increase as a bifurcation is approached~\citep{Livina2012, Dakos2011a}.
