...
to reflect the uncertainty inherent in the detection of early warning signals. We caution against paradigms that are not useful for capturing uncertainty in
...
restrict ourselves to systems that can produce a saddle-node bifurcation,
 approaches based on critical slowing down or rising variance (REFS) must further assume that a changing parameter
...
the calculation of statistics that would be used to detect an impending transition is subject to several additional arbitrary choices.
...
\subsection{A simulation-based approach estimates the distributions}
...
We illustrate this approach on simulated data as well as several natural time-series that have been previously analyzed for early warning signals.  All data and code for simulations and analysis are found in the accompanying R package, \verb|warningsignals|.
...
As described above, we simulate 500 replicates under each model, 
...
Asa way to clearly illustrate the choices involved in looking for warning signals while avoiding false alarms, we introduce an approach based on receiver operator curves. These curves XXX. The extent of the difficulty in finding reliable indicators of impending regime shifts based on summary statistics. becomes clear from the ROC curves of these statistics,
