...
by an explicit stochastic differential equation (SDE) whose coefficients are estimated from statistical moments of the data. 
...
We introduce a similar set of SDE models to these, 
\cb{should this next part come later?}
and go on to illustrate how these models can be fit to data by maximum likelihood and compared 
by a simulation or bootstrapping approach rooted in~\citet{Cox1961} and~\citet{McLachlan1987} that
characterizes the rate of missed detections and false alarms expected in the estimate 
(or equivalently, estimates both the power and statistical significance of the test).  

\subsection{Models}
The normal form for the saddle-node bifurcation is
\begin{equation}
